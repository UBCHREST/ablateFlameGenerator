...
void ablateFlameGenerator::VariableChange::Initialize(const ablate::domain::Domain& domain) {
...
bool ablateFlameGenerator::VariableChange::CheckConvergence(const ablate::domain::Domain& domain, PetscReal time, PetscInt step, const std::shared_ptr<ablate::monitors::logs::Log>& log) {
    // Get the subdomain for this region
    auto subDomain = domain.GetSubDomain(region);
...
REGISTER(ablateFlameGenerator::ConvergenceCriteria, ablateFlameGenerator::VariableChange, "This class checks for a relative change in the specified variable between checks",
