...
ablateFlameGenerator::ValidRange::ValidRange(std::string variableName, double lowerBound, double upperBound, std::shared_ptr<ablate::domain::Region> region)
    : variableName(std::move(variableName)), lowerBound(lowerBound), upperBound(upperBound), region(std::move(region)) {}

bool ablateFlameGenerator::ValidRange::CheckConvergence(const ablate::domain::Domain& domain, PetscReal time, PetscInt step, const std::shared_ptr<ablate::monitors::logs::Log>& log) {
...
REGISTER(ablateFlameGenerator::ConvergenceCriteria, ablateFlameGenerator::ValidRange, "This class will stop the convergence iterations if the bounds are exceeded",
